...
- **📋 计划管理**：每日交易计划 + 模板 + 标签 + 高频弱点预警 + AI 模糊表述检测
- **🔥 复盘拷打**：AI 流式交叉审计计划 vs 实际，5 级毒舌强度可调 + 快速复盘 + 复盘模板 + 复盘质量评分 + 自动纪律检查 + AI 追问
- **📊 弱点矩阵**：自动提取心理弱点，权重随时间演进，AI 模式分析 + 弱点关联图谱 + 衰减可视化
- **📏 交易纪律**：自定义规则 + AI 个性化生成 + 复盘时自动检测违反 + 违反历史追踪
- **📈 统计面板**：盈亏/胜率/回撤/情绪相关性/星期几表现 + 计划执行率趋势 + 情绪趋势
- **🔬 深度分析**：盈亏分布/情绪相关性/计划执行率/弱点时间线/交易时段/弱点关联图谱/情绪热力图/弱点衰减/情境分析
- **🤖 AI 增强**：本周vs上周对比/危险信号/弱点深度关联/个性化纪律/复盘模板推荐/纪律违反检测/盘前提醒/周度进步报告
- **💬 AI 教练**：随时对话，基于完整用户画像（弱点+盈亏+纪律+目标）给出建议
- **🎮 等级系统**：交易员XP/等级系统，复盘+完成计划+里程碑获得经验值
- **🏷️ 情境标签**：给复盘添加交易情境标签，按情境分析表现
- **🗓️ 交易日历**：月度 PnL 可视化 + 计划完成状态
- **📢 飞书通知**：每日推送(含风险指数) + 连亏预警 + 大额提醒 + 计划未完成提醒
- **📓 交易日记**：自由记录 + AI 总结 + 本周精华摘要
- **🎯 习惯追踪**：目标系统 + 连续复盘天数 + 里程碑
- **📊 仪表盘**：今日状态一览（计划/盈亏/连续天数/风险）
...
- **v9.0.0** — 第二大脑2.0：纪律违反追踪/交易员等级系统(XP)/AI复盘追问/情绪热力图/弱点衰减可视化/交易情境标签/AI盘前提醒/AI周度进步报告/情境分析/复盘对比/飞书通知增强
- **v8.0.0** — 行为改变引擎2.0：复盘模板系统/AI模板推荐/复盘质量评分/弱点关联图谱/自动纪律检查/日记周报/仪表盘/AI教练增强/移动端优化/飞书风险指数
